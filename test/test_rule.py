...
class TestCall(unittest.TestCase):
    ...
    def test_args(self):
        self.assertEqual(rules.Call.parse('a(b c d)'), ['a', ['b', 'c', 'd']])


class TestCommandTag(unittest.TestCase):
    def test_simple(self):
        self.assertEqual(rules.CommandTag.parse('[a]'), [types.Directive('a')])

    def test_call(self):
        self.assertEqual(
            rules.CommandTag.parse('[a()]'), [types.Directive('a')])
